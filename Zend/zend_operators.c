		...
		case IS_OBJECT:
			{
				...
				if (Z_IS_FALSE(dst) || Z_IS_TRUE(dst)) {
					ZVAL_COPY_VALUE(op, &dst);
				} else {
					ZVAL_TRUE(op);
				}
...
ZEND_API int ZEND_FASTCALL boolean_xor_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	do {
		if (Z_IS_FALSE_P(op2)) {
			op2_val = 0;
		} else if (EXPECTED(Z_IS_TRUE_P(op2))) {
			op2_val = 1;
		} else {
			if (Z_ISREF_P(op2)) {
				op2 = Z_REFVAL_P(op2);
...
ZEND_API int ZEND_FASTCALL boolean_not_function(zval *result, zval *op1) /* {{{ */
{
	if (Z_IS_TRUE_P(op1)) {
		ZVAL_FALSE(result);
	} else if (EXPECTED(Z_IS_LESS_THAN_TRUE_P(op1))) {
		ZVAL_TRUE(result);
	} else {
		if (Z_ISREF_P(op1)) {
			op1 = Z_REFVAL_P(op1);
...
ZEND_API int ZEND_FASTCALL bitwise_or_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (EXPECTED(Z_IS_LONG_P(op1)) && EXPECTED(Z_IS_LONG_P(op2))) {
		ZVAL_LONG(result, Z_LVAL_P(op1) | Z_LVAL_P(op2));
		return SUCCESS;
	}
	...
	if (Z_IS_STRING_P(op1) && EXPECTED(Z_IS_STRING_P(op2))) {
		zval *longer, *shorter;
		zend_string *str;
		size_t i;
...
ZEND_API int ZEND_FASTCALL bitwise_and_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	if (Z_IS_STRING_P(op1) && Z_IS_STRING_P(op2)) {
		zval *longer, *shorter;
		zend_string *str;
		size_t i;
...
ZEND_API int ZEND_FASTCALL concat_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	...
	do {
	 	if (UNEXPECTED(!Z_IS_STRING_P(op1))) {
	 		if (Z_ISREF_P(op1)) {
	 			op1 = Z_REFVAL_P(op1);
	 			if (Z_IS_STRING_P(op1)) break;
	 		}
	...
	do {
		if (UNEXPECTED(!Z_IS_STRING_P(op2))) {
	 		if (Z_ISREF_P(op2)) {
	 			op2 = Z_REFVAL_P(op2);
	 			if (Z_IS_STRING_P(op2)) break;
	 		}
	...
	while (1) {
		switch (TYPE_PAIR(Z_TYPE_P(op1), Z_TYPE_P(op2))) {
			...
			default:
				...
				if (Z_IS_OBJECT_P(op1) && Z_OBJ_HANDLER_P(op1, compare)) {
					ret = Z_OBJ_HANDLER_P(op1, compare)(result, op1, op2);
					if (UNEXPECTED(!Z_IS_LONG_P(result))) {
						convert_compare_result_to_long(result);
					}
					return ret;
				} else if (Z_IS_OBJECT_P(op2) && Z_OBJ_HANDLER_P(op2, compare)) {
				...
				if (Z_IS_OBJECT_P(op1) && Z_IS_OBJECT_P(op2)) {
				...
				if (!converted) {
					if (Z_IS_NULL_P(op1) || Z_IS_FALSE_P(op1)) {
						ZVAL_LONG(result, zval_is_true(op2) ? -1 : 0);
						return SUCCESS;
					} else if (Z_IS_NULL_P(op2) || Z_IS_FALSE_P(op2)) {
						ZVAL_LONG(result, zval_is_true(op1) ? 1 : 0);
						return SUCCESS;
					} else if (Z_IS_TRUE_P(op1)) {
						ZVAL_LONG(result, zval_is_true(op2) ? 0 : 1);
						return SUCCESS;
					} else if (Z_IS_TRUE_P(op2)) {
				...
				} else if (Z_IS_ARRAY_P(op1)) {
					ZVAL_LONG(result, 1);
					return SUCCESS;
				} else if (Z_IS_ARRAY_P(op2)) {
					ZVAL_LONG(result, -1);
					return SUCCESS;
				} else if (Z_IS_OBJECT_P(op1)) {
					ZVAL_LONG(result, 1);
					return SUCCESS;
				} else if (Z_IS_OBJECT_P(op2)) {
					ZVAL_LONG(result, -1);
					return SUCCESS;
				} else {
					ZVAL_LONG(result, 0);
