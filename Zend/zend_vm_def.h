...
ZEND_VM_HANDLER(1, ZEND_ADD, CONST|TMPVAR|CV, CONST|TMPVAR|CV)
{
	...
	SAVE_OPLINE();
	if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(op1))) {
		op1 = GET_OP1_UNDEF_CV(op1, BP_VAR_R);
	}
	if (OP2_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(op2))) {
...
ZEND_VM_HANDLER(8, ZEND_CONCAT, CONST|TMPVAR|CV, CONST|TMPVAR|CV, SPEC(NO_CONST_CONST))
{
	...
	if ((OP1_TYPE == IS_CONST || EXPECTED(Z_IS_STRING_P(op1))) &&
	    (OP2_TYPE == IS_CONST || EXPECTED(Z_IS_STRING_P(op2)))) {
...
ZEND_VM_HELPER(zend_binary_assign_op_obj_helper, VAR|UNUSED|CV, CONST|TMPVAR|CV, binary_op_type binary_op)
{
	...
	if (OP1_TYPE == IS_UNUSED && UNEXPECTED(Z_IS_UNDEF_P(object))) {
	...
	do {
		...
		if (OP1_TYPE != IS_UNUSED && UNEXPECTED(!Z_IS_OBJECT_P(object))) {
			ZVAL_DEREF(object);
		...
		if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr)
			&& EXPECTED((zptr = Z_OBJ_HT_P(object)->get_property_ptr_ptr(object, property, BP_VAR_RW, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL))) != NULL)) {
			...
			zend_assign_op_overloaded_property(object, property, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL), value, binary_op, (UNEXPECTED(RETURN_VALUE_USED(opline)) ? EX_VAR(opline->result.var) : NULL));
		}
	} while (0);
...
ZEND_VM_C_LABEL(assign_dim_op_new_array):
		...
		} else if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(container))) {
...
ZEND_VM_C_LABEL(assign_dim_op_convert_to_array):
		...
		if (EXPECTED(Z_IS_OBJECT_P(container))) {
			...
			if (UNEXPECTED(Z_IS_STRING_P(container))) {
				...
				UNDEF_RESULT();
			} else if (EXPECTED(Z_IS_LESS_THAN_TRUE_P(container))) {
...
ZEND_VM_HELPER(zend_pre_incdec_property_helper, VAR|UNUSED|CV, CONST|TMPVAR|CV, int inc)
{
	...
	do {
		...
		if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr)
			...
			zend_pre_incdec_overloaded_property(object, property, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL), inc, (UNEXPECTED(RETURN_VALUE_USED(opline)) ? EX_VAR(opline->result.var) : NULL));
		}
	} while (0);
...
ZEND_VM_HELPER(zend_post_incdec_property_helper, VAR|UNUSED|CV, CONST|TMPVAR|CV, int inc)
{
	...
	do {
		...
		if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr)
			...
			zend_post_incdec_overloaded_property(object, property, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL), inc, EX_VAR(opline->result.var));
		}
	} while (0);
...
ZEND_VM_HANDLER(34, ZEND_PRE_INC, VAR|CV, ANY, SPEC(RETVAL))
{
	...
	if (EXPECTED(Z_IS_LONG_P(var_ptr))) {
	...
	SAVE_OPLINE();
	if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(var_ptr))) {
...
ZEND_VM_HANDLER(40, ZEND_ECHO, CONST|TMPVAR|CV, ANY)
{
	...
	if (Z_IS_STRING_P(z)) {
		...
		if (ZSTR_LEN(str) != 0) {
			zend_write(ZSTR_VAL(str), ZSTR_LEN(str));
		} else if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(z))) {
...
ZEND_VM_C_LABEL(fetch_this):
			...
			switch (type) {
				case BP_VAR_R:
					if (EXPECTED(Z_IS_OBJECT(EX(This)))) {
	...
	} else if (Z_IS_INDIRECT_P(retval)) {
...
ZEND_VM_HANDLER(84, ZEND_FETCH_DIM_W, VAR|CV, CONST|TMPVAR|UNUSED|NEXT|CV)
{
	...
	result = EX_VAR(opline->result.var);
	zend_fetch_dimension_address_W(result, container, GET_OP2_ZVAL_PTR_UNDEF(BP_VAR_R), OP2_TYPE EXECUTE_DATA_CC);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR) {
...
ZEND_VM_HANDLER(87, ZEND_FETCH_DIM_RW, VAR|CV, CONST|TMPVAR|UNUSED|NEXT|CV)
{
	...
	result = EX_VAR(opline->result.var);
	zend_fetch_dimension_address_RW(result, container, GET_OP2_ZVAL_PTR_UNDEF(BP_VAR_R), OP2_TYPE EXECUTE_DATA_CC);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR) {
		FREE_VAR_PTR_AND_EXTRACT_RESULT_IF_NECESSARY(free_op1, result);
	}
...
ZEND_VM_HANDLER(96, ZEND_FETCH_DIM_UNSET, VAR|CV, CONST|TMPVAR|CV)
{
	...
	result = EX_VAR(opline->result.var);
	zend_fetch_dimension_address_UNSET(result, container, GET_OP2_ZVAL_PTR_UNDEF(BP_VAR_R), OP2_TYPE EXECUTE_DATA_CC);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR) {
		FREE_VAR_PTR_AND_EXTRACT_RESULT_IF_NECESSARY(free_op1, result);
	}
...
ZEND_VM_HOT_OBJ_HANDLER(82, ZEND_FETCH_OBJ_R, CONST|TMPVAR|UNUSED|THIS|CV, CONST|TMPVAR|CV)
{
	...
	if (OP1_TYPE == IS_UNUSED && UNEXPECTED(Z_IS_UNDEF_P(container))) {
		ZEND_VM_DISPATCH_TO_HELPER(zend_this_not_in_object_context_helper);
	}
	...
	if (OP1_TYPE == IS_CONST ||
	    (OP1_TYPE != IS_UNUSED && UNEXPECTED(!Z_IS_OBJECT_P(container)))) {
	    do {
			...
			if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(container))) {
				GET_OP1_UNDEF_CV(container, BP_VAR_R);
			}
			if (OP2_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(offset))) {
	...
	do {
		...
		if (OP2_TYPE == IS_CONST) {
			cache_slot = CACHE_ADDR(opline->cache_slot);
		...
		} else if (OP2_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(offset))) {
			GET_OP2_UNDEF_CV(offset, BP_VAR_R);
		}
...
ZEND_VM_HANDLER(85, ZEND_FETCH_OBJ_W, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV)
{
	...
	property = GET_OP2_ZVAL_PTR(BP_VAR_R);
	result = EX_VAR(opline->result.var);
	zend_fetch_property_address(result, container, OP1_TYPE, property, OP2_TYPE, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL), BP_VAR_W);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR) {
		FREE_VAR_PTR_AND_EXTRACT_RESULT_IF_NECESSARY(free_op1, result);
	}
...
ZEND_VM_HANDLER(88, ZEND_FETCH_OBJ_RW, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV)
{
	...
	property = GET_OP2_ZVAL_PTR(BP_VAR_R);
	result = EX_VAR(opline->result.var);
	zend_fetch_property_address(result, container, OP1_TYPE, property, OP2_TYPE, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL), BP_VAR_RW);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR) {
		FREE_VAR_PTR_AND_EXTRACT_RESULT_IF_NECESSARY(free_op1, result);
	}
...
ZEND_VM_HANDLER(97, ZEND_FETCH_OBJ_UNSET, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV)
{
	...
	property = GET_OP2_ZVAL_PTR(BP_VAR_R);
	result = EX_VAR(opline->result.var);
	zend_fetch_property_address(result, container, OP1_TYPE, property, OP2_TYPE, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL), BP_VAR_UNSET);
	FREE_OP2();
	if (OP1_TYPE == IS_VAR) {
		FREE_VAR_PTR_AND_EXTRACT_RESULT_IF_NECESSARY(free_op1, result);
	}
...
ZEND_VM_HANDLER(198, ZEND_FETCH_LIST_W, VAR|CV, CONST|TMPVAR|CV)
{
	...
	if (OP1_TYPE == IS_VAR
		&& !Z_IS_INDIRECT_P(EX_VAR(opline->op1.var))
...
ZEND_VM_HANDLER(136, ZEND_ASSIGN_OBJ, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, SPEC(OP_DATA=CONST|TMP|VAR|CV))
{
	...
	if (OP1_TYPE != IS_UNUSED && UNEXPECTED(!Z_IS_OBJECT_P(object))) {
		do {
			if (Z_ISREF_P(object)) {
				...
				if (EXPECTED(Z_IS_OBJECT_P(object))) {
					break;
				}
			}
			if (EXPECTED(Z_IS_LESS_THAN_TRUE_P(object) ||
			    (Z_IS_STRING_P(object) && Z_STRLEN_P(object) == 0))) {
				zend_object *obj;
	...
	if (OP2_TYPE == IS_CONST &&
	    EXPECTED(Z_OBJCE_P(object) == CACHED_PTR(opline->cache_slot))) {
		uintptr_t prop_offset = (uintptr_t)CACHED_PTR(opline->cache_slot + sizeof(void*));
		zend_object *zobj = Z_OBJ_P(object);
		zval *property_val;

		if (EXPECTED(IS_VALID_PROPERTY_OFFSET(prop_offset))) {
			property_val = OBJ_PROP(zobj, prop_offset);
			if (!Z_IS_UNDEF_P(property_val)) {
ZEND_VM_C_LABEL(fast_assign_obj):
	...
	Z_OBJ_HT_P(object)->write_property(object, property, value, (OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL);
...
ZEND_VM_HANDLER(147, ZEND_ASSIGN_DIM, VAR|CV, CONST|TMPVAR|UNUSED|NEXT|CV, SPEC(OP_DATA=CONST|TMP|VAR|CV))
{
	...
	if (EXPECTED(Z_IS_ARRAY_P(object_ptr))) {
ZEND_VM_C_LABEL(try_assign_dim_array):
		...
		if (EXPECTED(Z_IS_OBJECT_P(object_ptr))) {
			dim = GET_OP2_ZVAL_PTR(BP_VAR_R);
			...
			FREE_OP_DATA();
		} else if (EXPECTED(Z_IS_STRING_P(object_ptr))) {
			if (OP2_TYPE == IS_UNUSED) {
				zend_throw_error(NULL, "[] operator not supported for strings");
				FREE_UNFETCHED_OP_DATA();
				FREE_OP1_VAR_PTR();
		...
		} else if (EXPECTED(Z_IS_LESS_THAN_TRUE_P(object_ptr))) {
...
ZEND_VM_HANDLER(39, ZEND_ASSIGN_REF, VAR|CV, VAR|CV, SRC)
{
	...
	if (OP1_TYPE == IS_VAR &&
	    UNEXPECTED(!Z_IS_INDIRECT_P(EX_VAR(opline->op1.var))) &&
...
ZEND_VM_HANDLER(53, ZEND_FAST_CONCAT, CONST|TMPVAR|CV, CONST|TMPVAR|CV)
{
	...
	if (OP2_TYPE == IS_CONST) {
		op2_str = Z_STR_P(op2);
	} else if (EXPECTED(Z_IS_STRING_P(op2))) {
...
ZEND_VM_HANDLER(54, ZEND_ROPE_INIT, UNUSED, CONST|TMPVAR|CV, NUM)
{
	...
	if (OP2_TYPE == IS_CONST) {
		...
		if (EXPECTED(Z_IS_STRING_P(var))) {
			...
			SAVE_OPLINE();
			if (OP2_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(var))) {
...
ZEND_VM_C_LABEL(try_class_name):
		...
		} else if ((OP2_TYPE & (IS_VAR|IS_CV)) && Z_IS_REFERENCE_P(class_name)) {
			class_name = Z_REFVAL_P(class_name);
			ZEND_VM_C_GOTO(try_class_name);
		} else {
			if (OP2_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(class_name))) {
...
ZEND_VM_HOT_OBJ_HANDLER(112, ZEND_INIT_METHOD_CALL, CONST|TMPVAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, NUM)
{
	...
	if (OP2_TYPE != IS_CONST &&
		...
		do {
			if ((OP2_TYPE & (IS_VAR|IS_CV)) && Z_ISREF_P(function_name)) {
				...
				if (EXPECTED(Z_IS_STRING_P(function_name))) {
					break;
				}
			} else if (OP2_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(function_name))) {
	...
	if (OP1_TYPE != IS_UNUSED) {
		do {
			if (OP1_TYPE == IS_CONST || UNEXPECTED(!Z_IS_OBJECT_P(object))) {
				...
				if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(object))) {
	...
	if (OP2_TYPE == IS_CONST &&
	    EXPECTED(CACHED_PTR(opline->cache_slot) == called_scope)) {
	    fbc = CACHED_PTR(opline->cache_slot + sizeof(void*));
	} else {
		...
		if (OP2_TYPE == IS_CONST &&
			...
			CACHE_POLYMORPHIC_PTR(opline->cache_slot, called_scope, fbc);
		}
...
ZEND_VM_HANDLER(113, ZEND_INIT_STATIC_METHOD_CALL, UNUSED|CLASS_FETCH|CONST|VAR, CONST|TMPVAR|UNUSED|CONSTRUCTOR|CV, NUM)
{
	...
	if (OP1_TYPE == IS_CONST &&
	    OP2_TYPE == IS_CONST &&
	    EXPECTED((fbc = CACHED_PTR(opline->cache_slot + sizeof(void*))) != NULL)) {
		/* nothing to do */
		...
		function_name = GET_OP2_ZVAL_PTR_UNDEF(BP_VAR_R);
		if (OP2_TYPE != IS_CONST) {
			if (UNEXPECTED(!Z_IS_STRING_P(function_name))) {
				do {
					if (OP2_TYPE & (IS_VAR|IS_CV) && Z_ISREF_P(function_name)) {
						function_name = Z_REFVAL_P(function_name);
						if (EXPECTED(Z_IS_STRING_P(function_name))) {
		...
		if (Z_IS_OBJECT(EX(This)) && Z_OBJ(EX(This))->ce != ce->constructor->common.scope && (ce->constructor->common.fn_flags & ZEND_ACC_PRIVATE)) {
	...
	if (!(fbc->common.fn_flags & ZEND_ACC_STATIC)) {
		if (Z_IS_OBJECT(EX(This)) && instanceof_function(Z_OBJCE(EX(This)), ce)) {
...
ZEND_VM_HOT_HANDLER(59, ZEND_INIT_FCALL_BY_NAME, ANY, CONST, NUM)
{
	...
	if (UNEXPECTED(fbc == NULL)) {
		...
		CACHE_PTR(opline->cache_slot, fbc);
	}
	call = zend_vm_stack_push_call_frame(ZEND_CALL_NESTED_FUNCTION,
...
ZEND_VM_C_LABEL(try_function_name):
	if (OP2_TYPE != IS_CONST && EXPECTED(Z_IS_STRING_P(function_name))) {
		call = zend_init_dynamic_call_string(Z_STR_P(function_name), opline->extended_value);
	} else if (OP2_TYPE != IS_CONST && EXPECTED(Z_IS_OBJECT_P(function_name))) {
		call = zend_init_dynamic_call_object(function_name, opline->extended_value);
	} else if (EXPECTED(Z_IS_ARRAY_P(function_name))) {
		call = zend_init_dynamic_call_array(Z_ARRVAL_P(function_name), opline->extended_value);
	} else if ((OP2_TYPE & (IS_VAR|IS_CV)) && EXPECTED(Z_IS_REFERENCE_P(function_name))) {
		function_name = Z_REFVAL_P(function_name);
		ZEND_VM_C_GOTO(try_function_name);
	} else {
		if (OP2_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(function_name))) {
			GET_OP2_UNDEF_CV(function_name, BP_VAR_R);
			if (UNEXPECTED(EG(exception) != NULL)) {
				HANDLE_EXCEPTION();
			}
...
/* prevents "undefined variable opline" errors */
		...
		if (OP1_TYPE == IS_CONST) {
			...
			if (UNEXPECTED(Z_IS_INDIRECT_P(retval_ptr))) {
...
ZEND_VM_HOT_HANDLER(62, ZEND_RETURN, CONST|TMP|VAR|CV, ANY)
{
	...
	if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(retval_ptr))) {
		SAVE_OPLINE();
...
ZEND_VM_HANDLER(108, ZEND_THROW, CONST|TMP|VAR|CV, ANY)
{
	...
	do {
		if (OP1_TYPE == IS_CONST || UNEXPECTED(!Z_IS_OBJECT_P(value))) {
			if ((OP1_TYPE & (IS_VAR|IS_CV)) && Z_ISREF_P(value)) {
				value = Z_REFVAL_P(value);
				if (EXPECTED(Z_IS_OBJECT_P(value))) {
					break;
				}
			}
			if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(value))) {
...
ZEND_VM_HANDLER(107, ZEND_CATCH, CONST, CV, JMP_ADDR)
{
	...
	catch_ce = CACHED_PTR(opline->cache_slot);
	if (UNEXPECTED(catch_ce == NULL)) {
		...
		CACHE_PTR(opline->cache_slot, catch_ce);
	}
	ce = EG(exception)->ce;
...
ZEND_VM_HOT_HANDLER(117, ZEND_SEND_VAR, VAR|CV, NUM)
{
	...
	if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(varptr))) {
		SAVE_OPLINE();
...
ZEND_VM_C_LABEL(send_again):
	if (EXPECTED(Z_IS_ARRAY_P(args))) {
	...
	} else if (EXPECTED(Z_IS_OBJECT_P(args))) {
		...
		if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(args))) {
...
ZEND_VM_HANDLER(119, ZEND_SEND_ARRAY, ANY, ANY, NUM)
{
	...
	if (UNEXPECTED(!Z_IS_ARRAY_P(args))) {
...
ZEND_VM_HOT_HANDLER(64, ZEND_RECV_INIT, NUM, CONST)
{
	...
	if (UNEXPECTED((EX(func)->op_array.fn_flags & ZEND_ACC_HAS_TYPE_HINTS) != 0)) {
		...
		SAVE_OPLINE();
		if (UNEXPECTED(!zend_verify_arg_type(EX(func), arg_num, param, default_value, CACHE_ADDR(opline->cache_slot)) || EG(exception))) {
			HANDLE_EXCEPTION();
		}
	}
...
ZEND_VM_HANDLER(110, ZEND_CLONE, CONST|TMPVAR|UNUSED|THIS|CV, ANY)
{
	...
	if (OP1_TYPE == IS_UNUSED && UNEXPECTED(Z_IS_UNDEF_P(obj))) {
		ZEND_VM_DISPATCH_TO_HELPER(zend_this_not_in_object_context_helper);
	}

	do {
		if (OP1_TYPE == IS_CONST ||
		    (OP1_TYPE != IS_UNUSED && UNEXPECTED(!Z_IS_OBJECT_P(obj)))) {
		    if ((OP1_TYPE & (IS_VAR|IS_CV)) && Z_ISREF_P(obj)) {
				obj = Z_REFVAL_P(obj);
				if (EXPECTED(Z_IS_OBJECT_P(obj))) {
					break;
				}
			}
			ZVAL_UNDEF(EX_VAR(opline->result.var));
			if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(obj))) {
...
ZEND_VM_HANDLER(99, ZEND_FETCH_CONSTANT, UNUSED|CONST_FETCH, CONST)
{
	...
	if (EXPECTED(CACHED_PTR(opline->cache_slot))) {
		c = CACHED_PTR(opline->cache_slot);
...
ZEND_VM_HANDLER(181, ZEND_FETCH_CLASS_CONSTANT, VAR|CONST|UNUSED|CLASS_FETCH, CONST)
{
	...
	do {
		if (OP1_TYPE == IS_CONST) {
			if (EXPECTED(CACHED_PTR(opline->cache_slot + sizeof(void*)))) {
				value = CACHED_PTR(opline->cache_slot + sizeof(void*));
				break;
			} else if (EXPECTED(CACHED_PTR(opline->cache_slot))) {
				ce = CACHED_PTR(opline->cache_slot);
			} else {
				ce = zend_fetch_class_by_name(Z_STR_P(RT_CONSTANT(opline, opline->op1)), RT_CONSTANT(opline, opline->op1) + 1, ZEND_FETCH_CLASS_DEFAULT | ZEND_FETCH_CLASS_EXCEPTION);
				if (UNEXPECTED(ce == NULL)) {
			...
			if (EXPECTED(CACHED_PTR(opline->cache_slot) == ce)) {
				value = CACHED_PTR(opline->cache_slot + sizeof(void*));
				break;
			}
		}
...
ZEND_VM_C_LABEL(add_again):
		if (EXPECTED(Z_IS_STRING_P(offset))) {
			str = Z_STR_P(offset);
			if (OP2_TYPE != IS_CONST) {
...
ZEND_VM_C_LABEL(str_index):
		...
		} else if (EXPECTED(Z_IS_LONG_P(offset))) {
			hval = Z_LVAL_P(offset);
ZEND_VM_C_LABEL(num_index):
			zend_hash_index_update(Z_ARRVAL_P(EX_VAR(opline->result.var)), hval, expr_ptr);
		} else if ((OP2_TYPE & (IS_VAR|IS_CV)) && EXPECTED(Z_IS_REFERENCE_P(offset))) {
			offset = Z_REFVAL_P(offset);
			ZEND_VM_C_GOTO(add_again);
		} else if (Z_IS_NULL_P(offset)) {
			str = ZSTR_EMPTY_ALLOC();
			ZEND_VM_C_GOTO(str_index);
		} else if (Z_IS_DOUBLE_P(offset)) {
			hval = zend_dval_to_lval(Z_DVAL_P(offset));
			ZEND_VM_C_GOTO(num_index);
		} else if (Z_IS_FALSE_P(offset)) {
			hval = 0;
			ZEND_VM_C_GOTO(num_index);
		} else if (Z_IS_TRUE_P(offset)) {
			hval = 1;
			ZEND_VM_C_GOTO(num_index);
		} else if (OP2_TYPE == IS_CV && Z_IS_UNDEF_P(offset)) {
			GET_OP2_UNDEF_CV(offset, BP_VAR_R);
			str = ZSTR_EMPTY_ALLOC();
			ZEND_VM_C_GOTO(str_index);
		} else {
...
ZEND_VM_HANDLER(73, ZEND_INCLUDE_OR_EVAL, CONST|TMPVAR|CV, ANY, EVAL)
{
	...
	if (UNEXPECTED(EG(exception) != NULL)) {
		...
		call = zend_vm_stack_push_call_frame(ZEND_CALL_NESTED_CODE | ZEND_CALL_HAS_SYMBOL_TABLE,
			...
			!Z_IS_OBJECT(EX(This)) ? Z_CE(EX(This)) : NULL,
			Z_IS_OBJECT(EX(This)) ? Z_OBJ(EX(This)) : NULL);
...
ZEND_VM_C_LABEL(num_index_dim):
				...
				hval = 1;
				ZEND_VM_C_GOTO(num_index_dim);
			} else if (Z_IS_RESOURCE_P(offset)) {
		...
		} else if (OP1_TYPE != IS_UNUSED && UNEXPECTED(Z_IS_STRING_P(container))) {
...
ZEND_VM_HANDLER(76, ZEND_UNSET_OBJ, VAR|UNUSED|THIS|CV, CONST|TMPVAR|CV)
{
	...
	do {
		if (OP1_TYPE != IS_UNUSED && UNEXPECTED(!Z_IS_OBJECT_P(container))) {
			if (Z_ISREF_P(container)) {
				container = Z_REFVAL_P(container);
				if (!Z_IS_OBJECT_P(container)) {
					break;
				}
			} else {
				break;
		...
		if (Z_OBJ_HT_P(container)->unset_property) {
			Z_OBJ_HT_P(container)->unset_property(container, offset, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL));
		} else {
...
ZEND_VM_HANDLER(77, ZEND_FE_RESET_R, CONST|TMP|VAR|CV, JMP_ADDR)
{
	...
	if (EXPECTED(Z_IS_ARRAY_P(array_ptr))) {
		result = EX_VAR(opline->result.var);
		...
		FREE_OP1_IF_VAR();
		ZEND_VM_NEXT_OPCODE();
	} else if (OP1_TYPE != IS_CONST && EXPECTED(Z_IS_OBJECT_P(array_ptr))) {
...
ZEND_VM_HANDLER(78, ZEND_FE_FETCH_R, VAR, ANY, JMP_ADDR)
{
	...
	SAVE_OPLINE();
	if (EXPECTED(Z_IS_ARRAY_P(array))) {
	...
	} else if (EXPECTED(Z_IS_OBJECT_P(array))) {
		zend_object_iterator *iter;
...
ZEND_VM_HANDLER(180, ZEND_ISSET_ISEMPTY_STATIC_PROP, CONST|TMPVAR|CV, UNUSED|CLASS_FETCH|CONST|VAR, ISSET)
{
	...
	if (OP2_TYPE == IS_CONST) {
		...
		} else if (UNEXPECTED((ce = CACHED_PTR(opline->cache_slot)) == NULL)) {
			ce = zend_fetch_class_by_name(Z_STR_P(RT_CONSTANT(opline, opline->op2)), RT_CONSTANT(opline, opline->op2) + 1, ZEND_FETCH_CLASS_DEFAULT | ZEND_FETCH_CLASS_EXCEPTION);
			if (UNEXPECTED(ce == NULL)) {
				ZEND_ASSERT(EG(exception));
				ZVAL_UNDEF(EX_VAR(opline->result.var));
...
ZEND_VM_C_LABEL(num_index_prop):
	...
	if ((OP1_TYPE != IS_CONST && EXPECTED(Z_IS_OBJECT_P(container)))) {
	...
	} else if (EXPECTED(Z_IS_STRING_P(container))) { /* string offsets */
		zend_long lval;

		if (EXPECTED(Z_IS_LONG_P(offset))) {
...
ZEND_VM_HANDLER(148, ZEND_ISSET_ISEMPTY_PROP_OBJ, CONST|TMPVAR|UNUSED|THIS|CV, CONST|TMPVAR|CV, ISSET)
{
	...
	if (OP1_TYPE == IS_CONST ||
		...
		if ((OP1_TYPE & (IS_VAR|IS_CV)) && Z_ISREF_P(container)) {
			container = Z_REFVAL_P(container);
			if (UNEXPECTED(!Z_IS_OBJECT_P(container))) {
...
ZEND_VM_C_LABEL(isset_no_object):
		...
		result =
			((opline->extended_value & ZEND_ISSET) == 0) ^
			Z_OBJ_HT_P(container)->has_property(container, offset, (opline->extended_value & ZEND_ISSET) == 0, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->cache_slot) : NULL));
	}
...
ZEND_VM_C_LABEL(try_instanceof):
	...
	} else if ((OP1_TYPE & (IS_VAR|IS_CV)) && Z_IS_REFERENCE_P(expr)) {
		expr = Z_REFVAL_P(expr);
		ZEND_VM_C_GOTO(try_instanceof);
	} else {
		if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(expr))) {
...
ZEND_VM_HANDLER(144, ZEND_ADD_INTERFACE, ANY, CONST)
{
	...
	SAVE_OPLINE();
	iface = CACHED_PTR(opline->cache_slot);
	if (UNEXPECTED(iface == NULL)) {
		...
		CACHE_PTR(opline->cache_slot, iface);
	}
...
ZEND_VM_HANDLER(154, ZEND_ADD_TRAIT, ANY, ANY)
{
	...
	SAVE_OPLINE();
	trait = CACHED_PTR(opline->cache_slot);
	if (UNEXPECTED(trait == NULL)) {
		...
		CACHE_PTR(opline->cache_slot, trait);
	}
...
ZEND_VM_HANDLER(142, ZEND_YIELD_FROM, CONST|TMP|VAR|CV, ANY)
{
	...
	if (Z_IS_ARRAY_P(val)) {
		...
		FREE_OP1_IF_VAR();
	} else if (OP1_TYPE != IS_CONST && Z_IS_OBJECT_P(val) && Z_OBJCE_P(val)->get_iterator) {
...
ZEND_VM_HOT_HANDLER(168, ZEND_BIND_GLOBAL, CV, CONST)
{
	...
	idx = (uintptr_t)CACHED_PTR(opline->cache_slot) - 1;
	...
	if (UNEXPECTED(value == NULL)) {
		...
		CACHE_PTR(opline->cache_slot, (void*)(idx + 1));
	} else {
		idx = (char*)value - (char*)EG(symbol_table).arData;
		/* Store "hash slot index" + 1 (NULL is a mark of uninitialized cache slot) */
		CACHE_PTR(opline->cache_slot, (void*)(idx + 1));
ZEND_VM_C_LABEL(check_indirect):
		...
		if (UNEXPECTED(Z_IS_INDIRECT_P(value))) {
			value = Z_INDIRECT_P(value);
			if (UNEXPECTED(Z_IS_UNDEF_P(value))) {
				ZVAL_NULL(value);
			}
		}
	}
...
ZEND_VM_HANDLER(121, ZEND_STRLEN, CONST|TMPVAR|CV, ANY)
{
	...
	value = GET_OP1_ZVAL_PTR_UNDEF(BP_VAR_R);
	if (EXPECTED(Z_IS_STRING_P(value))) {
		...
		if ((OP1_TYPE & (IS_VAR|IS_CV)) && Z_IS_REFERENCE_P(value)) {
			value = Z_REFVAL_P(value);
			if (EXPECTED(Z_IS_STRING_P(value))) {
				ZVAL_LONG(EX_VAR(opline->result.var), Z_STRLEN_P(value));
				FREE_OP1();
...
ZEND_VM_C_LABEL(type_check_resource):
		if (EXPECTED(!Z_IS_RESOURCE_P(value))
	...
	} else if (OP1_TYPE == IS_CV && UNEXPECTED(Z_IS_UNDEF_P(value))) {
...
ZEND_VM_HANDLER(183, ZEND_BIND_STATIC, CV, CONST, REF)
{
	...
	if (opline->extended_value) {
		...
		if (UNEXPECTED(!Z_ISREF_P(value))) {
			...
			Z_REF_P(value) = ref;
			Z_SET_TYPE_INFO_P(value, IS_REFERENCE_EX);
			ZVAL_REF(variable_ptr, ref);
		} else {
			Z_ADDREF_P(value);
...
ZEND_VM_HANDLER(49, ZEND_CHECK_VAR, CV, UNUSED)
{
	...
	if (UNEXPECTED(Z_IS_UNDEF_P(op1))) {
		SAVE_OPLINE();
		GET_OP1_UNDEF_CV(op1, BP_VAR_R);
		ZEND_VM_NEXT_OPCODE_CHECK_EXCEPTION();
	}
...
ZEND_VM_HANDLER(51, ZEND_MAKE_REF, VAR|CV, UNUSED)
{
	...
	} else if (EXPECTED(Z_IS_INDIRECT_P(op1))) {
...
ZEND_VM_C_LABEL(fetch_dim_r_index_array):
	...
	} else if (OP1_TYPE != IS_CONST && EXPECTED(Z_IS_REFERENCE_P(container))) {
		container = Z_REFVAL_P(container);
		if (EXPECTED(Z_IS_ARRAY_P(container))) {
