...
static int choose_branch(zend_jit_func_info *info, zend_worklist *in, int i)
{
	...
	s0 = info->cfg.blocks[i].successors[0];
	s1 = info->cfg.blocks[i].successors[1];

	if (s1 < 0)
		return 0;

	s0_loop = info->cfg.blocks[s0].loop_header;
	s1_loop = info->cfg.blocks[s1].loop_header;
...
static int compute_block_map(zend_jit_func_info *info, struct block_order *order, int *block_map)
{
	...
	for (next = 0; next < info->cfg.blocks_count; ) {
...
static int compute_block_order(zend_op_array *op_array, struct block_order *order, int *block_map, int *live_blocks)
{
	...
	ZEND_ASSERT(info->cfg.blocks[0].start == 0);
	...
	while (zend_worklist_len(&in)) {
		...
		zend_basic_block *block = info->cfg.blocks + i;
...
//??? Block sorter leads to generation of incorrect code
	...
	order = alloca(sizeof(struct block_order) * info->cfg.blocks_count);

	/* block_map: an array mapping old block_num to new block_num.  */
	block_map = alloca(sizeof(int) * info->cfg.blocks_count);
	FILL_ARRAY(block_map, -1, int, info->cfg.blocks_count);

	if (compute_block_order(op_array, order, block_map, &blocks_count) != SUCCESS)
		return FAILURE;

	ZEND_ASSERT(blocks_count <= info->cfg.blocks_count);

	/* Check if block order is still the same */
	if (blocks_count == info->cfg.blocks_count) {
		for (i = 0; i < blocks_count; i++) {
			if (block_map[i] != i) {
				break;
			}
	...
	ZEND_JIT_CONTEXT_CALLOC(ctx, blocks, blocks_count);
...
static int zend_jit_compute_use_def_chains(zend_jit_context *ctx, zend_op_array *op_array)
{
	...
	if (!info->ssa.vars) {
		ZEND_JIT_CONTEXT_CALLOC(ctx, info->ssa.vars, info->ssa.vars_count);
	}
	ssa_vars = info->ssa.vars;
	if (!info->ssa_var_info) {
		ZEND_JIT_CONTEXT_CALLOC(ctx, info->ssa_var_info, info->ssa.vars_count);
	}
...
static int zend_jit_compute_false_dependencies(zend_op_array *op_array)
{
	...
	for (i = 0; i < ssa_vars_count; i++) {
		...
		use = info->ssa.vars[i].use_chain;
		while (use >= 0) {
			if (op_array->opcodes[use].opcode != ZEND_ASSIGN ||
				info->ssa.ops[use].op1_use != i ||
				info->ssa.ops[use].op2_use == i) {
				ssa_vars[i].no_val = 0; /* used directly */
				zend_bitset_incl(worklist, i);
			}				
			use = next_use(ssa_ops, i, use);			
		}
	}
		...
		if (ssa_vars[i].definition_phi) {
			...
			if (p->pi >= 0) {
				...
				for (j = 0; j < info->cfg.blocks[p->block].predecessors_count; j++) {
					if (p->sources[j] >= 0 && info->ssa.vars[p->sources[j]].no_val) {
...
static int zend_jit_compute_preallocated_cvs(zend_op_array *op_array)
{
	...
	for (i = 0; i < (uint32_t)op_array->last_var; i++) {
		if (info->ssa.vars[i].no_val &&
		    i != EX_VAR_TO_NUM(op_array->this_var)) {
			phi = info->ssa.vars[i].phi_use_chain;
			while (phi) {
				if (info->cfg.blocks[phi->block].flags & ZEND_BB_LOOP_HEADER) {
	...
	for (i = 0; i < op_array->last_var; i++) {
		if (info->ssa_var_info[i].type & MAY_BE_DEF &&
		    info->ssa.vars[i].var != EX_VAR_TO_NUM(op_array->this_var)) {
...
static void zend_jit_determine_preallocated_cv_type(zend_op_array *op_array, int var, int v)
{
	...
	if (info->ssa.vars[v].use_chain >= 0) {
		...
		op = &info->ssa.ops[info->ssa.vars[v].use_chain];
	...
	phi = info->ssa.vars[v].phi_use_chain;
	if (phi && !phi->visited) {
		phi->visited = 1;
		while (phi && next_use_phi(info, v, phi) != info->ssa.vars[v].phi_use_chain) {
			zend_jit_determine_preallocated_cv_type(op_array, var, phi->ssa_var);
			phi = next_use_phi(info, v, phi);
		}
		info->ssa.vars[v].phi_use_chain->visited = 0;
	}
}
...
#define CHECK_SCC_VAR(var2) \
	do { \
		if (!info->ssa.vars[var2].no_val) { \
			if (dfs[var2] < 0) { \
				zend_jit_check_scc_var(op_array, var2, index, dfs, root, stack); \
			} \
			if (info->ssa.vars[var2].scc < 0 && dfs[root[var]] >= dfs[root[var2]]) { \
			    root[var] = root[var2]; \
			} \
		} \
	} while (0)
...
#define CHECK_SCC_ENTRY(var2) \
	do { \
		if (info->ssa.vars[var2].scc != info->ssa.vars[var].scc) { \
			info->ssa.vars[var2].scc_entry = 1; \
		} \
	} while (0)

#define ADD_SCC_VAR(_var) \
	do { \
		if (info->ssa.vars[_var].scc == scc) { \
...
#define FOR_EACH_VAR_USAGE(_var, MACRO) \
	do { \
		...
		while (use >= 0) { \
			...
			use = next_use(info->ssa.ops, _var, use); \
		} \
...
static int zend_jit_calc_range(zend_jit_context *ctx, zend_op_array *op_array, int var, int widening, int narrowing, zend_jit_range *tmp)
{
	...
	if (info->ssa.vars[var].definition_phi) {
		...
		if (p->pi >= 0) {
			...
			for (i = 0; i < info->cfg.blocks[p->block].predecessors_count; i++) {
	...
	} else if (info->ssa.vars[var].definition < 0) {
	...
	line = info->ssa.vars[var].definition;
	...
	switch (opline->opcode) {
		case ZEND_ADD:
			if (info->ssa.ops[line].result_def == var) {
...
//		case ZEND_BW_XOR:
		...
		case ZEND_ASSERT_CHECK:
			...
			break;
		case ZEND_SEND_VAR:
			if (info->ssa.ops[line].op1_def == var) {
				if (info->ssa.ops[line].op1_def >= 0) {
					if (OP1_HAS_RANGE()) {
						tmp->underflow = OP1_RANGE_UNDERFLOW();
						tmp->min = OP1_MIN_RANGE();
			...
			break;
		case ZEND_PRE_INC:
			if (info->ssa.ops[line].op1_def == var || info->ssa.ops[line].result_def == var) {
				if (OP1_HAS_RANGE()) {
					tmp->min = OP1_MIN_RANGE();
					tmp->max = OP1_MAX_RANGE();
					tmp->underflow = OP1_RANGE_UNDERFLOW();
		...
		case ZEND_UNSET_OBJ:
			...
			break;
		case ZEND_ASSIGN:
			if (info->ssa.ops[line].op1_def == var || info->ssa.ops[line].op2_def == var || info->ssa.ops[line].result_def == var) {
				if (OP2_HAS_RANGE()) {
		...
		case ZEND_ASSIGN_OBJ:
			if (info->ssa.ops[line+1].op1_def == var) {
...
static void zend_jit_check_scc_var(zend_op_array *op_array, int var, int *index, int *dfs, int *root, zend_worklist_stack *stack)
{
	...
	p = info->ssa.vars[var].sym_use_chain;
	while (p) {
	...
	if (root[var] == var) {
		info->ssa.vars[var].scc = info->sccs;
		while (stack->len > 0) {
			...
			info->ssa.vars[var2].scc = info->sccs;
		}
		info->sccs++;
	} else {
...
static int zend_jit_find_sccs(zend_op_array *op_array)
{
	...
	dfs = alloca(sizeof(int) * info->ssa.vars_count);
	memset(dfs, -1, sizeof(int) * info->ssa.vars_count);
	root = alloca(sizeof(int) * info->ssa.vars_count);
	ZEND_WORKLIST_STACK_ALLOCA(&stack, info->ssa.vars_count);

	/* Find SCCs */
	for (j = 0; j < info->ssa.vars_count; j++) {
		if (!info->ssa.vars[j].no_val && dfs[j] < 0) {
	...
	for (j = 0; j < info->ssa.vars_count; j++) {
		if (info->ssa.vars[j].scc >= 0) {
			info->ssa.vars[j].scc = info->sccs - (info->ssa.vars[j].scc + 1);
		}
	}
...
#ifdef LOG_SSA_RANGE
	fprintf(stderr, "  change range (init      SCC %2d) %2d [%s%ld..%ld%s]\n", info->ssa.vars[var].scc, var, (underflow?"-- ":""), min, max, (overflow?" ++":""));
#endif
}
...
{
	zend_jit_func_info *info = JIT_DATA(op_array);
	int worklist_len = zend_bitset_len(info->ssa.vars_count);
		...
		while (!zend_bitset_empty(worklist, worklist_len)) {
			...
			if (zend_jit_calc_range(ctx, op_array, j, 0, 0, &tmp)) {
				...
				if (!has_inner_cycles &&
				    ...
				    info->ssa.vars[j].definition_phi &&
				    info->ssa.vars[j].definition_phi->pi >= 0 &&
				    info->ssa.vars[j].definition_phi->constraint.negative &&
				    info->ssa.vars[j].definition_phi->constraint.min_ssa_var < 0 &&
				    info->ssa.vars[j].definition_phi->constraint.min_ssa_var < 0) {
					if (tmp.min == info->ssa_var_info[j].range.min &&
					    tmp.max == info->ssa_var_info[j].range.max) {
						if (info->ssa.vars[j].definition_phi->constraint.negative == NEG_INIT) {
#ifdef LOG_NEG_RANGE
							fprintf(stderr, "#%d INVARIANT\n", j);
#endif
							info->ssa.vars[j].definition_phi->constraint.negative = NEG_INVARIANT;
						}
					} else if (tmp.min == info->ssa_var_info[j].range.min &&
					           tmp.max == info->ssa_var_info[j].range.max + 1 &&
					           tmp.max < info->ssa.vars[j].definition_phi->constraint.range.min) {
						if (info->ssa.vars[j].definition_phi->constraint.negative == NEG_INIT ||
						    info->ssa.vars[j].definition_phi->constraint.negative == NEG_INVARIANT) {
#ifdef LOG_NEG_RANGE
							fprintf(stderr, "#%d LT\n", j);
#endif
							info->ssa.vars[j].definition_phi->constraint.negative = NEG_USE_LT;
						} else if (!info->ssa.vars[j].definition_phi->constraint.negative == NEG_USE_GT) {
#ifdef LOG_NEG_RANGE
							fprintf(stderr, "#%d UNKNOWN\n", j);
#endif
							info->ssa.vars[j].definition_phi->constraint.negative = NEG_UNKNOWN;
						}
					} else if (tmp.max == info->ssa_var_info[j].range.max &&
				               tmp.min == info->ssa_var_info[j].range.min - 1 &&
					           tmp.min > info->ssa.vars[j].definition_phi->constraint.range.max) {
						if (info->ssa.vars[j].definition_phi->constraint.negative == NEG_INIT ||
						    info->ssa.vars[j].definition_phi->constraint.negative == NEG_INVARIANT) {
#ifdef LOG_NEG_RANGE
							fprintf(stderr, "#%d GT\n", j);
#endif
							info->ssa.vars[j].definition_phi->constraint.negative = NEG_USE_GT;
						} else if (!info->ssa.vars[j].definition_phi->constraint.negative == NEG_USE_LT) {
#ifdef LOG_NEG_RANGE
							fprintf(stderr, "#%d UNKNOWN\n", j);
#endif
							info->ssa.vars[j].definition_phi->constraint.negative = NEG_UNKNOWN;
...
static int zend_jit_infer_ranges(zend_jit_context *ctx, zend_op_array *op_array)
{
	...
	for (j = 0; j < info->ssa.vars_count; j++) {
		if (info->ssa.vars[j].scc >= 0) {
			next_scc_var[j] = scc_var[info->ssa.vars[j].scc];
			scc_var[info->ssa.vars[j].scc] = j;
		}
	}		
...
static void add_usages(zend_op_array *op_array, zend_bitset worklist, int var)
{
	...
	if (info->ssa.vars[var].use_chain >= 0) {
		int use = info->ssa.vars[var].use_chain;
		do {
			if (info->ssa.ops[use].result_def >= 0) {
				zend_bitset_incl(worklist, info->ssa.ops[use].result_def);
			}
			if (info->ssa.ops[use].op1_def >= 0) {
				zend_bitset_incl(worklist, info->ssa.ops[use].op1_def);
			}
			if (info->ssa.ops[use].op2_def >= 0) {
				zend_bitset_incl(worklist, info->ssa.ops[use].op2_def);
			}
			if (op_array->opcodes[use].opcode == ZEND_OP_DATA) {
				if (info->ssa.ops[use-1].result_def >= 0) {
					zend_bitset_incl(worklist, info->ssa.ops[use-1].result_def);
				}
				if (info->ssa.ops[use-1].op1_def >= 0) {
					zend_bitset_incl(worklist, info->ssa.ops[use-1].op1_def);
				}
				if (info->ssa.ops[use-1].op2_def >= 0) {
					zend_bitset_incl(worklist, info->ssa.ops[use-1].op2_def);
				}
			}
			use = next_use(info->ssa.ops, var, use);
		} while (use >= 0);
	}
}
...
static void reset_dependent_vars(zend_op_array *op_array, zend_bitset worklist, int var)
{
	...
	while (use >= 0) {
		if (ssa_ops[use].op1_def >= 0 && ssa_var_info[ssa_ops[use].op1_def].type) {
			...
			reset_dependent_vars(op_array, worklist, ssa_ops[use].op1_def);
		}
		if (ssa_ops[use].op2_def >= 0 && ssa_var_info[ssa_ops[use].op2_def].type) {
			...
			reset_dependent_vars(op_array, worklist, ssa_ops[use].op2_def);
		}
		if (ssa_ops[use].result_def >= 0 && ssa_var_info[ssa_ops[use].result_def].type) {
			...
			reset_dependent_vars(op_array, worklist, ssa_ops[use].result_def);
		}
		if (op_array->opcodes[use+1].opcode == ZEND_OP_DATA) {
			if (ssa_ops[use+1].op1_def >= 0 && ssa_var_info[ssa_ops[use+1].op1_def].type) {
				...
				reset_dependent_vars(op_array, worklist, ssa_ops[use+1].op1_def);
			}
			if (ssa_ops[use+1].op2_def >= 0 && ssa_var_info[ssa_ops[use+1].op2_def].type) {
				...
				reset_dependent_vars(op_array, worklist, ssa_ops[use+1].op2_def);
			}
			if (ssa_ops[use+1].result_def >= 0 && ssa_var_info[ssa_ops[use+1].result_def].type) {
				...
				reset_dependent_vars(op_array, worklist, ssa_ops[use+1].result_def);
			}
		}
		use = next_use(ssa_ops, var, use);
	}
#ifdef SYM_RANGE
	/* Process symbolic control-flow constraints */
	p = info->ssa.vars[var].sym_use_chain;
		...
		case ZEND_FETCH_DIM_FUNC_ARG:
			if (ssa_ops[i].op1_def >= 0) {
				...
				while (j >= 0) {
					...
					j = next_use(ssa_ops, ssa_ops[i].result_def, j);
				}
				UPDATE_SSA_TYPE(tmp, ssa_ops[i].op1_def);
				if ((t1 & MAY_BE_OBJECT) && ssa_ops[i].op1_use >= 0 && ssa_var_info[ssa_ops[i].op1_use].ce) {
					UPDATE_SSA_OBJ_TYPE(ssa_var_info[ssa_ops[i].op1_use].ce, ssa_var_info[ssa_ops[i].op1_use].is_instanceof, ssa_ops[i].op1_def);
		...
		if (blocks[j].flags & ZEND_BB_REACHABLE) {
			...
			if (opline->opcode == ZEND_RETURN || opline->opcode == ZEND_RETURN_BY_REF) {
				if (!recursive &&
				    ...
				    info->ssa_var_info &&
				    info->ssa.ops[opline - op_array->opcodes].op1_use >= 0 &&
				    info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].recursive) {
					continue;
				}
				...
				if (info->ssa.ops &&
				    info->ssa_var_info &&
				    info->ssa.ops[opline - op_array->opcodes].op1_use >= 0 &&
				    info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].ce) {
					arg_ce = info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].ce;
					arg_is_instanceof = info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].is_instanceof;
				} else {
					arg_ce = NULL;
					arg_is_instanceof = 0;
				}
				...
				if (opline->op1_type == IS_CONST) {
					if (Z_TYPE_P(RT_CONSTANT(op_array, opline->op1)) == IS_NULL) {
						...
						tmp_has_range = 0;
				           ...
				           info->ssa_var_info &&
				           info->ssa.ops[opline - op_array->opcodes].op1_use >= 0) {
					if (info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].has_range) {
						if (tmp_has_range < 0) {
							tmp_has_range = 1;
							tmp_range = info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].range;
						} else if (tmp_has_range) {
							/* union */
							if (info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].range.underflow) {
								tmp_range.underflow = 1;
								tmp_range.min = LONG_MIN;
							} else {
								tmp_range.min = MIN(tmp_range.min, info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].range.min);
							}
							if (info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].range.overflow) {
								tmp_range.overflow = 1;
								tmp_range.max = LONG_MAX;
							} else {
								tmp_range.max = MAX(tmp_range.max, info->ssa_var_info[info->ssa.ops[opline - op_array->opcodes].op1_use].range.max);
							}
						}
					} else if (!widening) {
						tmp_has_range = 1;
		...
		do {
			if (call_info->num_args >= info->num_args &&
				...
				if ((OP1_INFO() & MAY_BE_OBJECT) &&
				    ...
				    caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].ce) {

					if (tmp_is_instanceof < 0) {
						tmp_ce = caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].ce;
						tmp_is_instanceof = caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].is_instanceof;
					} else if (caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].ce &&
					           caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].ce == tmp_ce) {
						if (caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].is_instanceof) {
							tmp_is_instanceof = 1;
						}
					} else {
						tmp_ce = NULL;
				...
				if (opline->op1_type == IS_CONST) {
					if (Z_TYPE_P(RT_CONSTANT(op_array, opline->op1)) == IS_NULL) {
						...
						tmp_has_range = 0;
				           ...
				           caller_info->ssa_var_info &&
				           caller_info->ssa.ops[line].op1_use >= 0) {
					if (caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].has_range) {
						if (tmp_has_range < 0) {
							tmp_has_range = 1;
							tmp_range = caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].range;
						} else if (tmp_has_range) {
							if (caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].range.underflow) {
								tmp_range.underflow = 1;
								tmp_range.min = LONG_MIN;
							} else {
								tmp_range.min = MIN(tmp_range.min, caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].range.min);
							}
							if (caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].range.overflow) {
								tmp_range.overflow = 1;
								tmp_range.max = LONG_MAX;
							} else {
								tmp_range.max = MAX(tmp_range.max, caller_info->ssa_var_info[caller_info->ssa.ops[line].op1_use].range.max);
							}
						}
					} else if (!widening) {
						tmp_has_range = 1;
...
#define CHECK_MAY_BE_USED_AS_DOUBLE(var2) do { \
		if ((info->ssa_var_info[var2].type & MAY_BE_ANY) == (MAY_BE_LONG | MAY_BE_DOUBLE)) { \
			if (info->ssa.vars[var2].var < op_array->last_var) { \
				return 0; \
			} else if (info->ssa.vars[var2].definition >= 0 && \
			           op_array->opcodes[info->ssa.vars[var2].definition].opcode != ZEND_ADD && \
			           op_array->opcodes[info->ssa.vars[var2].definition].opcode != ZEND_SUB && \
			           op_array->opcodes[info->ssa.vars[var2].definition].opcode != ZEND_MUL && \
			           op_array->opcodes[info->ssa.vars[var2].definition].opcode != ZEND_DIV) { \
				return 0; \
	...
	for (j = op_array->last_var; j < ssa_vars_count; j++) {
		if (ssa_vars[j].var < op_array->last_var &&
			...
			if ((ssa_var_info[j].type & MAY_BE_ANY) == MAY_BE_LONG &&
			    ...
			    op_array->opcodes[ssa_vars[j].definition].opcode == ZEND_ASSIGN &&
			    op_array->opcodes[ssa_vars[j].definition].op2_type == IS_CONST &&
			    Z_TYPE_P(RT_CONSTANT(op_array, op_array->opcodes[ssa_vars[j].definition].op2)) == IS_LONG) {
...
static void zend_jit_check_no_symtab(zend_op_array *op_array)
{
	...
	for (b = 0; b < info->cfg.blocks_count; b++) {
		if ((info->cfg.blocks[b].flags & ZEND_BB_REACHABLE) == 0) {
			continue;
		}
		for (i = info->cfg.blocks[b].start; i <= info->cfg.blocks[b].end; i++) {
			zend_op *opline = op_array->opcodes + i;
				...
				case ZEND_MOD:
					if (opline->op2_type == IS_CONST) {
						...
						if (info->ssa.ops[i].op2_use < 0 ||
						    !info->ssa_var_info[info->ssa.ops[i].op2_use].has_range ||
						    (info->ssa_var_info[info->ssa.ops[i].op2_use].range.min <= 0 &&
						     info->ssa_var_info[info->ssa.ops[i].op2_use].range.max >= 0)) {
							return;
						}
					}
					goto check_ops;
...
check_ops:
					if (opline->op1_type == IS_CV &&
					    ...
					    (info->ssa_var_info[info->ssa.ops[i].op1_use].type & MAY_BE_UNDEF)) {
					    return;
					} else if (opline->op1_type == IS_TMP_VAR &&
					    info->ssa.ops[i].op1_use >= 0 &&
					    (info->ssa_var_info[info->ssa.ops[i].op1_use].type & (MAY_BE_ARRAY_OF_ARRAY|MAY_BE_ARRAY_OF_OBJECT|MAY_BE_ARRAY_OF_RESOURCE|MAY_BE_OBJECT|MAY_BE_RESOURCE))) {
					    return;
					} else if (opline->op1_type == IS_VAR &&
					    info->ssa.ops[i].op1_use >= 0 &&
					    (info->ssa_var_info[info->ssa.ops[i].op1_use].type & MAY_BE_RC1) &&
					    (info->ssa_var_info[info->ssa.ops[i].op1_use].type & (MAY_BE_ARRAY_OF_ARRAY|MAY_BE_ARRAY_OF_OBJECT|MAY_BE_ARRAY_OF_RESOURCE|MAY_BE_OBJECT|MAY_BE_RESOURCE))) {
					    return;
					}
check_op2:
					if (opline->op2_type == IS_TMP_VAR &&
					    ...
					    (info->ssa_var_info[info->ssa.ops[i].op2_use].type & (MAY_BE_ARRAY_OF_ARRAY|MAY_BE_ARRAY_OF_OBJECT|MAY_BE_ARRAY_OF_RESOURCE|MAY_BE_OBJECT|MAY_BE_RESOURCE))) {
					    return;
					} else if (opline->op2_type == IS_VAR &&
					    info->ssa.ops[i].op2_use >= 0 &&
					    (info->ssa_var_info[info->ssa.ops[i].op2_use].type & MAY_BE_RC1) &&
					    (info->ssa_var_info[info->ssa.ops[i].op2_use].type & (MAY_BE_ARRAY_OF_ARRAY|MAY_BE_ARRAY_OF_OBJECT|MAY_BE_ARRAY_OF_RESOURCE|MAY_BE_OBJECT|MAY_BE_RESOURCE))) {
					    return;
					}
check_cv2:
					if (opline->op2_type == IS_CV &&
					    info->ssa.ops[i].op2_use >= 0 &&
					    (info->ssa_var_info[info->ssa.ops[i].op2_use].type & MAY_BE_UNDEF)) {
					    return;
					}
					break;
check_op1:
...
//???
				...
				use = next_use(info->ssa.ops, j, use);
			}
		}
	...
	if (ZCG(accel_directives).jit_debug & JIT_DEBUG_DUMP_TYPES) {
		...
		for (i = 0; i < ip_vars; i++) {
			...
			if (ip_var[i].kind == IP_VAR) {
				...
				zend_jit_dump_var(ctx->op_arrays[ip_var[i].op_array_num], JIT_DATA(ctx->op_arrays[ip_var[i].op_array_num])->ssa.vars[ip_var[i].num].var);
				fprintf(stderr, ")");
