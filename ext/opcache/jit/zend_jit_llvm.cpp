                                 ...
                                 bool           tail_call)
{
	...
	if (tail_call) {
		if (llvm_ctx.inline_level) {
			...
			zend_func_info *info = ZEND_FUNC_INFO(llvm_ctx.op_array);
			if (info->clone_num) {
				if (info->return_info.type & MAY_BE_IN_REG) {
					if (info->return_info.type & MAY_BE_DOUBLE) {
...
static BasicBlock *zend_jit_find_exception_bb(zend_llvm_ctx &ctx, zend_op *opline)
{
	...
	if (catch_bb_num >= 0) {
		if (!ctx.bb_exceptions[catch_bb_num]) {
			...
			const zend_func_info *info = ZEND_FUNC_INFO(ctx.op_array);
			for (int i = 0; i < info->ssa.cfg.blocks_count; i++) {
				if (info->ssa.cfg.blocks[i].start == catch_op_num) {
                                   ...
                                   uint32_t       info)
{
	if (op_type == IS_CONST) {
	    ...
	    ((ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[ssa_var].type & (MAY_BE_UNDEF|MAY_BE_ANY)) == MAY_BE_LONG) &&
		ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[ssa_var].has_range &&
		ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[ssa_var].range.min ==
			ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[ssa_var].range.max) {
		return LLVM_GET_LONG(ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[ssa_var].range.min);
	} else {
	...
	if (info & MAY_BE_UNDEF) {
		...
		switch (mode) {
			...
			case BP_VAR_UNSET: {
				...
				if (check && !(ZEND_FUNC_INFO(llvm_ctx.op_array)->flags & ZEND_JIT_FUNC_NO_FRAME)) {
...
static int zend_jit_needs_check_for_this(zend_llvm_ctx &llvm_ctx, int bb)
{
	...
	ZEND_ASSERT(bb >= 0 && bb < info->ssa.cfg.blocks_count);
	if (!zend_bitset_in(llvm_ctx.this_checked, bb)) {
		zend_bitset_incl(llvm_ctx.this_checked, bb);
		while (bb != info->ssa.cfg.blocks[bb].idom) {
			bb = info->ssa.cfg.blocks[bb].idom;
			if (bb < 0) {
				return 1;
...
{
	zend_ssa *ssa = &ZEND_FUNC_INFO(op_array)->ssa;
                                       ...
                                       uint32_t          from_info)
{
	...
	zend_ssa *ssa = &ZEND_FUNC_INFO(op_array)->ssa;
	zend_func_info *info = ZEND_FUNC_INFO(op_array);
	Value *to_addr = zend_jit_load_slot(llvm_ctx, (zend_uintptr_t)ZEND_CALL_VAR_NUM(NULL, info->ssa.vars[from_var].var));
                               ...
                               uint32_t       from_info)
{
	...
	if (from_info & MAY_BE_UNDEF) {
		from_info |= MAY_BE_NULL;
	}
                                         ...
                                         zend_op          *opline)
{
	if (expr_op_type == IS_CONST) {
		...
		PHI_DCL(val, 3);
		if (expr_info & MAY_BE_UNDEF) {
			expr_info |= MAY_BE_NULL;
		}
		...
		
{
	...
	if (op_type == IS_UNUSED && op_array->scope) {
		...
		zend_ssa_var_info *op_info = (ssa_var >= 0)? &ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[ssa_var] : NULL;
...
numeric_dim:
		...
		if (dim_info & MAY_BE_UNDEF) {
			dim_info |= MAY_BE_NULL;
		}
                                                    ...
                                                    zend_bool         *may_threw)
{
	...
	if (container_info & MAY_BE_UNDEF) {
		container_info |= MAY_BE_NULL;
	}
                                               ...
                                               zend_bool         *may_threw)
{
	...
	if (container_info & MAY_BE_STRING) {
		...
		if (dim_op_type == IS_UNUSED) {
			// JIT: zend_throw_error(NULL, "[] operator not supported for strings");
			zend_jit_throw_error(llvm_ctx, opline,
					NULL, "[] operator not supported for strings");
			if (!*bb_error) {
				*bb_error = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
			}
			llvm_ctx.builder.CreateBr(*bb_error);
                           ...
                           int               expected_branch)
{
	...
	if (OP1_OP_TYPE() == IS_CONST) {
		...
		return 1;
	} else if (!OP1_MAY_BE((MAY_BE_UNDEF | MAY_BE_ANY) - MAY_BE_TRUE)) {
	...
	if (op1_info & MAY_BE_TRUE) {
		if (op1_info & (MAY_BE_ANY - MAY_BE_TRUE)) {
                        ...
                        int               expected_branch)
{
	...
	if (bb_slow_path) {
		...
		if (opline->opcode == ZEND_IS_IDENTICAL &&
			...
			if (!(op2_info & (MAY_BE_UNDEF | MAY_BE_NULL))) {
				cmp = llvm_ctx.builder.getInt1(0);
			} else if (!(op2_info & (MAY_BE_ANY-MAY_BE_NULL))) {
			...
			if (!(op2_info & MAY_BE_FALSE)) {
				cmp = llvm_ctx.builder.getInt1(0);
			} else if (!(op2_info & (MAY_BE_ANY-MAY_BE_FALSE))) {
				cmp = llvm_ctx.builder.getInt1(1);
			} else {
				Value *op2_type = zend_jit_load_type_c(llvm_ctx, op2_addr, OP2_OP_TYPE(), OP2_OP(), OP2_SSA_VAR(), op2_info);
				cmp = llvm_ctx.builder.CreateICmpEQ(
			...
			if (!(op2_info & MAY_BE_TRUE)) {
				cmp = llvm_ctx.builder.getInt1(0);
			} else if (!(op2_info & (MAY_BE_ANY-MAY_BE_TRUE))) {
				cmp = llvm_ctx.builder.getInt1(1);
			} else {
				Value *op2_type = zend_jit_load_type_c(llvm_ctx, op2_addr, OP2_OP_TYPE(), OP2_OP(), OP2_SSA_VAR(), op2_info);
				cmp = llvm_ctx.builder.CreateICmpEQ(
			...
			if (!(op1_info & MAY_BE_FALSE)) {
				cmp = llvm_ctx.builder.getInt1(0);
			} else if (!(op1_info & (MAY_BE_ANY-MAY_BE_FALSE))) {
				cmp = llvm_ctx.builder.getInt1(1);
			} else {
				Value *op1_type = zend_jit_load_type_c(llvm_ctx, op1_addr, OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), op1_info);
				cmp = llvm_ctx.builder.CreateICmpEQ(
			...
			if (!(op1_info & MAY_BE_TRUE)) {
				cmp = llvm_ctx.builder.getInt1(0);
			} else if (!(op1_info & (MAY_BE_ANY-MAY_BE_TRUE))) {
				cmp = llvm_ctx.builder.getInt1(1);
			} else {
				Value *op1_type = zend_jit_load_type_c(llvm_ctx, op1_addr, OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), op1_info);
				cmp = llvm_ctx.builder.CreateICmpEQ(
			...
			if (!llvm_ctx.valid_opline && ((op1_info & MAY_BE_OBJECT) || (op2_info & MAY_BE_OBJECT))) {
				zend_jit_store_opline(llvm_ctx, opline, false);
			}
		...
		if (opcode == ZEND_DIV) {
			...
			if (op1_op_type == IS_CONST &&
			    ...
			    op1_min = Z_LVAL_P(RT_CONSTANT(llvm_ctx.op_array, *op1_op));
			           ...
			           ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info &&
			           ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[op1_ssa_var].has_range) {
			    op1_min = ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[op1_ssa_var].range.min;
			} else {
				op1_min = LONG_MIN;
			}
			if (op2_op_type == IS_CONST &&
			    ...
			    op2_min = op2_max = Z_LVAL_P(RT_CONSTANT(llvm_ctx.op_array, *op2_op));
			           ...
			           ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info &&
			           ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[op2_ssa_var].has_range) {
			    op2_min = ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[op2_ssa_var].range.min;
			    op2_max = ZEND_FUNC_INFO(llvm_ctx.op_array)->ssa.var_info[op2_ssa_var].range.max;
			} else {
				op2_min = LONG_MIN;
				op2_max = LONG_MAX;
			}
                           ...
                           zend_op          *opline)
{
	...
	if (op1_info & MAY_BE_LONG) {
 	...
 	if (OP1_OP_TYPE() == IS_VAR && (op1_info & MAY_BE_ERROR)) {
		BasicBlock *bb_error = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
                                    ...
                                    zend_op          *opline)
{
	...
	if (opline->opcode == ZEND_SEND_VAR_NO_REF_EX) {
		//JIT: if (ARG_SHOULD_BE_SENT_BY_REF(EX(call)->func, OP2_OP()->num)) {
	...
	Value *op1_addr = zend_jit_load_operand(llvm_ctx,
				OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), OP1_INFO(), 0, opline);
	Value *op1_type_info = zend_jit_load_type_info_c(llvm_ctx, op1_addr, OP1_OP_TYPE(), OP1_OP(), OP1_SSA_VAR(), OP1_INFO());
	...
	//JIT: ZVAL_COPY_VALUE(arg, varptr);
	zend_jit_copy_value(llvm_ctx, arg_addr, 0, -1, MAY_BE_ANY,
		op1_addr, op1_type_info, OP1_OP_TYPE(), OP1_OP(),
		OP1_SSA_VAR(),
                               ...
                               zend_op           *opline)
{
	...
	if (opline->op1_type == IS_UNUSED && zend_jit_needs_check_for_this(llvm_ctx, bb)) {
		...
		zend_jit_unexpected_br(llvm_ctx,
			llvm_ctx.builder.CreateICmpEQ(
				zend_jit_load_type(llvm_ctx, op1_addr, -1, MAY_BE_ANY),
				llvm_ctx.builder.getInt8(IS_UNDEF)),
			bb_error,
			bb_follow);
		llvm_ctx.builder.SetInsertPoint(bb_error);
                              ...
                              zend_uchar         assign_type)
{
	...
	if (opline->extended_value == ZEND_ASSIGN_OBJ ||
	    (opline->extended_value == ZEND_ASSIGN_DIM && (op1_info & MAY_BE_OBJECT))) {
		return zend_jit_handler(llvm_ctx, opline);
	}
		...
		case ZEND_ASSIGN_DIM:
			{
				...
				if (op1_info & (MAY_BE_ANY - MAY_BE_OBJECT)) {
					...
					if (ret) {
						do_assign = 1;
						var_ptr = ret;
						var_info = zend_array_element_type(op1_info, 1, OP2_OP_TYPE() == IS_UNUSED) & ~MAY_BE_ERROR;
						var_ssa_var = -1;
						var_def_info = zend_array_element_type(OP1_DEF_INFO(), 1, OP2_OP_TYPE() == IS_UNUSED) & ~MAY_BE_ERROR;
						var_def_ssa_var = -1;
						if (bb_new_element) {
							PHI_ADD(var_ptr, ret);
                                          ...
                                          zend_op           *opline)
{
	...
	if (OP1_OP_TYPE() != IS_UNUSED) {
		...
		if (op1_info & MAY_BE_ARRAY) {
			...
			if (OP2_OP_TYPE() == IS_CONST) {
				...
				if (op2_info & MAY_BE_DOUBLE) {
					...
					if (op2_info & (MAY_BE_ANY-(MAY_BE_DOUBLE|MAY_BE_LONG|MAY_BE_STRING))) {
						BasicBlock *bb_double = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
						bb_cont = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
						if (!op2_type) {
							op2_type = zend_jit_load_type(llvm_ctx, offset, OP2_SSA_VAR(), op2_info);
				...
				if (op2_info & MAY_BE_NULL) {
					if (bb_cont) {
						llvm_ctx.builder.SetInsertPoint(bb_cont);
						bb_cont = NULL;
					}

					if (op2_info & (MAY_BE_ANY-(MAY_BE_NULL|MAY_BE_DOUBLE|MAY_BE_LONG|MAY_BE_STRING))) {
						BasicBlock *bb_null = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
						bb_cont = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
						if (!op2_type) {
							op2_type = zend_jit_load_type(llvm_ctx, offset, OP2_SSA_VAR(), op2_info);
				...
				if (op2_info & MAY_BE_FALSE) {
					...
					if (op2_info & (MAY_BE_ANY-(MAY_BE_FALSE|MAY_BE_NULL|MAY_BE_DOUBLE|MAY_BE_LONG|MAY_BE_STRING))) {
						BasicBlock *bb_false = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
						bb_cont = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
						if (!op2_type) {
							op2_type = zend_jit_load_type(llvm_ctx, offset, OP2_SSA_VAR(), op2_info);
		...
		if (op1_info & MAY_BE_OBJECT) {
			...
			if (op1_info & (MAY_BE_ANY-(MAY_BE_OBJECT|MAY_BE_ARRAY))) {
				BasicBlock *bb_object = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				bb_follow = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				if (!op1_type) {
					op1_type = zend_jit_load_type(llvm_ctx, container, OP1_SSA_VAR(), op1_info);
		...
		if (op1_info & MAY_BE_STRING) {
			...
			if (op1_info & (MAY_BE_ANY-(MAY_BE_STRING|MAY_BE_OBJECT|MAY_BE_ARRAY))) {
				BasicBlock *bb_string = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				bb_follow = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
				if (!op1_type) {
					op1_type = zend_jit_load_type(llvm_ctx, container, OP1_SSA_VAR(), op1_info);
		...
		if (bb_follow || (op1_info & (MAY_BE_ANY-(MAY_BE_STRING|MAY_BE_OBJECT|MAY_BE_ARRAY)))) {
			if (bb_follow) {
				llvm_ctx.builder.SetInsertPoint(bb_follow);
			}
                                                   ...
                                                   uint32_t          lineno)
{
	...
	llvm_ctx.builder.CreateAlignedStore(
		llvm_ctx.builder.getInt32(IS_UNDEF | (ZEND_CALL_TOP_FUNCTION << ZEND_CALL_INFO_SHIFT)),
		zend_jit_GEP(
			llvm_ctx,
			call,
...
/* {{{ static int zend_jit_free_call_frame_helper */
                                             ...
                                             uint32_t          lineno)
{
	...
	zend_jit_unexpected_br(llvm_ctx,
		llvm_ctx.builder.CreateICmpNE(
			llvm_ctx.builder.CreateAnd(
				call_info,
				llvm_ctx.builder.getInt32(ZEND_CALL_ALLOCATED << ZEND_CALL_INFO_SHIFT)),
			llvm_ctx.builder.getInt32(0)),
		bb_slow,
		bb_fast);
	...
	if (func && num_args != -1) {
		...
		if (num_args > first_extra_arg) {
			...
			if (src != dst) {
				...
				// TODO JIT: ZEND_ADD_CALL_FLAG(execute_data, ((type_flags >> Z_TYPE_FLAGS_SHIFT) & IS_TYPE_REFCOUNTED));
				// JIT: ZEND_ADD_CALL_FLAG(execute_data, ZEND_CALL_FREE_EXTRA_ARGS);
				llvm_ctx.builder.CreateStore(
					llvm_ctx.builder.CreateOr(
						llvm_ctx.builder.getInt32(ZEND_CALL_FREE_EXTRA_ARGS << ZEND_CALL_INFO_SHIFT),
						llvm_ctx.builder.CreateLoad(
							zend_jit_GEP(
								llvm_ctx,
								execute_data,
								offsetof(zend_execute_data, This.u1.type_info),
                                      ...
                                      int                 var)
{
	...
	if (info->ssa.vars[var].phi_use_chain) {		
		...
		do {
			...
			p = zend_ssa_next_use_phi(&info->ssa, var, p);
		} while (p);
	}
}
/* }}} */
                             ...
                             zend_op          *opline)
{
	...
	if (call_info) {
		if (call_info->callee_func) {
			...
			if (func && func->type == ZEND_USER_FUNCTION) {
				func_info = ZEND_FUNC_INFO(&func->op_array);
			}
		}
	}
	...
	Value *call = llvm_ctx.builder.CreateAlignedLoad(
			zend_jit_GEP(
				...
				PointerType::getUnqual(PointerType::getUnqual(llvm_ctx.zend_execute_data_type))), 4);
	if (!call_info ||
	    (call_info->caller_init_opline->opcode != ZEND_INIT_FCALL &&
	     call_info->caller_init_opline->opcode != ZEND_INIT_FCALL_BY_NAME &&
	     call_info->caller_init_opline->opcode != ZEND_INIT_NS_FCALL_BY_NAME)) {
		may_be_method = 1;
	}
...
//???			Value *ret = zend_jit_load_var(llvm_ctx, RES_OP()->var);
	...
	if (!func || func->type == ZEND_USER_FUNCTION) {
		...
		if (1) {
			...
			zend_jit_init_func_execute_data(llvm_ctx, call, func, func_addr, return_value, num_args, opline->lineno);
	...
	if (may_be_method) {
		...
		zend_jit_unexpected_br(llvm_ctx,
			llvm_ctx.builder.CreateICmpNE(
				llvm_ctx.builder.CreateAnd(
					call_info,
					llvm_ctx.builder.getInt32(ZEND_CALL_RELEASE_THIS << ZEND_CALL_INFO_SHIFT)),
			llvm_ctx.builder.getInt32(0)),
			bb_follow,
			bb_end);
		llvm_ctx.builder.SetInsertPoint(bb_follow);
...
static int zend_jit_free_compiled_variables(zend_llvm_ctx    &llvm_ctx,
                                            ...
                                            zend_op          *opline)
{
	zend_func_info *ctx = ZEND_FUNC_INFO(op_array);
    uint32_t info;

	// TODO: use type inference to avoid useless zval_ptr_dtor() ???...
	for (uint32_t i = 0 ; i < op_array->last_var; i++) {	    
	    if (ctx && ctx->ssa.vars && ctx->ssa.var_info) {
			info = ctx->ssa.var_info[i].type;
		    for (uint32_t j = op_array->last_var; j < ctx->ssa.vars_count; j++) {
		    	if (ctx->ssa.vars[j].var == i) {
		    		if (!(ctx->ssa.var_info[j].type & MAY_BE_IN_REG)) {
						info |= ctx->ssa.var_info[j].type;
					}
				}
			}
		} else {
...
/* {{{ static int zend_jit_free_extra_args_helper */
                          ...
                          zend_op          *opline)
{
	...
	zend_jit_unexpected_br(llvm_ctx,
		llvm_ctx.builder.CreateICmpNE(
			llvm_ctx.builder.CreateAnd(
				call_info,
				llvm_ctx.builder.getInt32((ZEND_CALL_HAS_SYMBOL_TABLE|ZEND_CALL_FREE_EXTRA_ARGS|ZEND_CALL_ALLOCATED) << ZEND_CALL_INFO_SHIFT)),
			llvm_ctx.builder.getInt32(0)),
		bb_slow,
		bb_fast);
	llvm_ctx.builder.SetInsertPoint(bb_slow);	
	...
	if ((info->flags & ZEND_JIT_FUNC_NO_SYMTAB) == 0) {
		...
		zend_jit_unexpected_br(llvm_ctx,
			llvm_ctx.builder.CreateICmpNE(
				llvm_ctx.builder.CreateAnd(
					call_info,
					llvm_ctx.builder.getInt32(ZEND_CALL_HAS_SYMBOL_TABLE << ZEND_CALL_INFO_SHIFT)),
				llvm_ctx.builder.getInt32(0)),
			bb_follow,
			bb_skip);
		llvm_ctx.builder.SetInsertPoint(bb_follow);
	...
	zend_jit_free_extra_args(llvm_ctx, call_info, op_array, opline);
	...
	zend_jit_vm_stack_free_call_frame(llvm_ctx, call_info, llvm_ctx._execute_data, opline->lineno);
...
{	
	...
	for (i = 0; i < info->ssa.vars_count; i++) {
		if (info->ssa.var_info[i].type & MAY_BE_IN_REG) {
			if (info->ssa.var_info[i].type & (MAY_BE_NULL|MAY_BE_FALSE|MAY_BE_TRUE)) {
				if (info->ssa.vars[i].var < op_array->last_var) {
					if (!tmp_reg[info->ssa.vars[i].var * 4 + 0]) {
						tmp_reg[info->ssa.vars[i].var * 4 + 0] = llvm_ctx.builder.CreateAlloca(Type::getInt32Ty(llvm_ctx.context));
					}
					reg = tmp_reg[info->ssa.vars[i].var * 4 + 0];
				} else {
					reg = llvm_ctx.builder.CreateAlloca(Type::getInt32Ty(llvm_ctx.context));
				}
			} else if (info->ssa.var_info[i].type & MAY_BE_LONG) {
				if (info->ssa.vars[i].var < op_array->last_var) {
					if (!tmp_reg[info->ssa.vars[i].var * 4 + 1]) {
						tmp_reg[info->ssa.vars[i].var * 4 + 1] = llvm_ctx.builder.CreateAlloca(Type::LLVM_GET_LONG_TY(llvm_ctx.context));
					}
					reg = tmp_reg[info->ssa.vars[i].var * 4 + 1];
				} else {
					reg = llvm_ctx.builder.CreateAlloca(Type::LLVM_GET_LONG_TY(llvm_ctx.context));
				}
			} else if (info->ssa.var_info[i].type & MAY_BE_DOUBLE) {
				if (info->ssa.vars[i].var < op_array->last_var) {
					if (!tmp_reg[info->ssa.vars[i].var * 4 + 2]) {
						tmp_reg[info->ssa.vars[i].var * 4 + 2] = llvm_ctx.builder.CreateAlloca(Type::getDoubleTy(llvm_ctx.context));
					}
					reg = tmp_reg[info->ssa.vars[i].var * 4 + 2];
				} else {
					reg = llvm_ctx.builder.CreateAlloca(Type::getDoubleTy(llvm_ctx.context));
				}
			} else if (info->ssa.var_info[i].type & MAY_BE_STRING) {
				reg = llvm_ctx.builder.CreateAlloca(PointerType::getUnqual(llvm_ctx.zend_string_type));
			} else if (info->ssa.var_info[i].type & MAY_BE_ARRAY) {
				reg = llvm_ctx.builder.CreateAlloca(PointerType::getUnqual(llvm_ctx.zend_array_type));
			} else if (info->ssa.var_info[i].type & MAY_BE_OBJECT) {
				reg = llvm_ctx.builder.CreateAlloca(PointerType::getUnqual(llvm_ctx.zend_object_type));
			} else if (info->ssa.var_info[i].type & MAY_BE_RESOURCE) {
                                       ...
                                       int               to_block)
{
	...
	if (info && info->ssa.var_info) {
		...
		zend_basic_block *b = info->ssa.cfg.blocks + to_block;
		...
		for (; p; p = p->next) {
			...
			if (p->pi >= 0) {
				...
				for (j = 0; j < b->predecessors_count; j++) {
					if (info->ssa.cfg.predecessors[b->predecessor_offset + j] == from_block) {
			...
			if (info->ssa.vars[from_var].no_val) {
				continue;
			}			
			to_info = info->ssa.var_info[to_var].type;
			from_info = info->ssa.var_info[from_var].type;
                               ...
                               zend_op_array    *op_array)
{
	...
	jit_stat.ssa_vars += info->ssa.vars_count - op_array->last_var;
	if (info->ssa.var_info) {
		for (i = op_array->last_var; i < info->ssa.vars_count; i++) {
			if ((info->ssa.var_info[i].type & MAY_BE_ANY) == MAY_BE_ANY) {
				jit_stat.untyped_ssa_vars++;
			} else if (has_concrete_type(info->ssa.var_info[i].type)) {
		...
		jit_stat.untyped_ssa_vars += info->ssa.vars_count - op_array->last_var;
	}
#endif
	...
	llvm_ctx.bb_labels = (BasicBlock**)zend_arena_calloc(&ctx->arena, info->ssa.cfg.blocks_count, sizeof(BasicBlock*));
	if (!llvm_ctx.bb_labels) return 0;

	if (op_array->last_try_catch) {
		llvm_ctx.bb_exceptions = (BasicBlock**)zend_arena_calloc(&ctx->arena, op_array->last_try_catch, sizeof(BasicBlock*));
		if (!llvm_ctx.bb_exceptions) return 0;
	}

	/* Find variables that may be allocated in registers */
	llvm_ctx.reg = (Value**)zend_arena_calloc(&ctx->arena, info->ssa.vars_count, sizeof(Value*));

//???	llvm_ctx.param_reg = (Value**)zend_arena_calloc(&ctx->arena, op_array->used_stack, sizeof(Value*));
...
//???	}
	...
	ZEND_ASSERT(!(blocks[0].flags & ZEND_BB_TARGET));
	for (b = 1; b < info->ssa.cfg.blocks_count; b++) {
		if (blocks[b].flags & ZEND_BB_REACHABLE) {
			llvm_ctx.bb_labels[b] = BasicBlock::Create(llvm_ctx.context, "", llvm_ctx.function);
		}
	}

	llvm_ctx.this_checked = (zend_bitset)zend_arena_calloc(&ctx->arena, zend_bitset_len(info->ssa.cfg.blocks_count), sizeof(zend_ulong));

	llvm_ctx.valid_opline = 1;

	from_b = -1;
	for (b = 0; b < info->ssa.cfg.blocks_count; b++) {
		if ((blocks[b].flags & ZEND_BB_REACHABLE) == 0) {
			continue;
		}
		if (b > 0) {
		...
		if (blocks[b].flags & ZEND_BB_TARGET) {
			llvm_ctx.valid_opline = 0;
		} else if (b > 0 && blocks[b - 1].start + blocks[b - 1].len != blocks[b].start) {
			llvm_ctx.valid_opline = 0;
		}

		from_b = b;
		if (!blocks[b].len) {
			if (blocks[b].successors[0] >= 0) {
				llvm_ctx.builder.CreateBr(TARGET_BB(blocks[b].successors[0]));
			}
			continue;
		}
		end = blocks[b].start + blocks[b].len - 1;
		for (i = blocks[b].start; i <= end; i++) {
			...
			switch (opline->opcode) {
				...
				case ZEND_JMPZ:
					if (!zend_jit_jmpznz(
							...
							op_array,
							opline,
							TARGET_BB(blocks[b].successors[0]),
							TARGET_BB(blocks[b].successors[1]),
							1)) return 0;
					llvm_ctx.valid_opline = 0;
					break;
				case ZEND_JMPNZ:
...
//???
				...
				case ZEND_DEFINED:
					...
					if ((opline+1)->opcode == ZEND_JMPZ || (opline+1)->opcode == ZEND_JMPNZ) {
						opline++;
						i++;
						if (!zend_jit_cond_jmp(llvm_ctx, opline, OP_JMP_ADDR(opline, *OP2_OP()), TARGET_BB(blocks[b].successors[0]), TARGET_BB(blocks[b].successors[1]))) return 0;
					}
					break;
#if 0
...
//					break;
				...
				case ZEND_FE_FETCH_RW:
					if (!zend_jit_handler(llvm_ctx, opline)) return 0;
					if (!zend_jit_cond_jmp(llvm_ctx, opline, ZEND_OFFSET_TO_OPLINE(opline, opline->extended_value), TARGET_BB(blocks[b].successors[0]), TARGET_BB(blocks[b].successors[1]))) return 0;
					break;
				case ZEND_THROW:
		...
		if (blocks[b].successors[0] >= 0 &&
		    blocks[b].successors[1] < 0 &&
//??? "i" or "b"?
		    blocks[b].successors[0] != i + 1) {
			switch (op_array->opcodes[end].opcode) {
				case ZEND_JMP:
					break;
				default:
					llvm_ctx.builder.CreateBr(TARGET_BB(blocks[b].successors[0]));
...
//	zend_basic_block      *blocks       = info->ssa.cfg.blocks;
	...
	zend_ssa_var_info *ssa_var_info = info->ssa.var_info;
	...
	for (i = 0; i < ssa_vars_count; i++) {
		...
		if (has_concrete_type(ssa_var_info[i].type) ||
			...
			if (ssa_vars[i].definition >= 0) {
				if (!zend_opline_supports_reg_alloc(op_array, op_array->opcodes + ssa_vars[i].definition, ssa_vars + i)) {
					continue;
				}
			}
			...
			j = ssa_vars[i].use_chain;
			while (j >= 0) {
				if (!zend_opline_supports_reg_alloc(op_array, op_array->opcodes + j, ssa_vars + i)) {
					ssa_var_info[i].type &= ~MAY_BE_IN_REG;
					break;
				}
				j = zend_ssa_next_use(ssa_ops, i, j);
			}
		}
	}
}
...
//???	if (info->flags & ZEND_JIT_FUNC_HAS_REG_ARGS) {
//???		llvm_ctx.arg_reg = (Value**)zend_arena_calloc(&ctx->arena, info->num_args, sizeof(Value*));
//???		int i;
//???
//???		for (i = 0 ; i < info->num_args; i++) {
