		...
		case IS_STRING:
			if (Z_STRLEN_P(data)) {
				...
				if (numbytes != Z_STRLEN_P(data)) {
					php_error_docref(NULL, E_WARNING, "Only "ZEND_SIZE_T_FMT" of %zd bytes written, possibly out of free disk space", numbytes, Z_STRLEN_P(data));
					numbytes = -1;
				}
			}
			break;
			...
			if (Z_OBJ_HT_P(data) != NULL) {
				...
				if (zend_std_cast_object_tostring(data, &out, IS_STRING) == SUCCESS) {
					...
					if (numbytes != Z_STRLEN(out)) {
						php_error_docref(NULL, E_WARNING, "Only "ZEND_SIZE_T_FMT" of %zd bytes written, possibly out of free disk space", numbytes, Z_STRLEN(out));
						numbytes = -1;
					}
					zval_dtor(&out);
					break;
...
PHPAPI PHP_FUNCTION(fread)
{
	...
	if (Z_STRLEN_P(return_value) < len / 2) {
		Z_SET_PTR2_P(return_value, IS_STRING_EX, zend_string_truncate(Z_STR_P(return_value), Z_STRLEN_P(return_value), 0));
	}
}
/* }}} */
